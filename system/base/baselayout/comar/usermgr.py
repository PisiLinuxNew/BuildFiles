...
def checkName(name):
    ...
    if len(name) == 0 or len([x for x in name if not x in valid]) != 0 or not name[0] in first_valid:
        fail(_(invalid_username_msg))

def checkRealName(realname):
    if len([x for x in realname if x == "\n" or x == ":"]) != 0:
...
def checkGroupName(name):
    ...
    if name == "" or len([x for x in name if not x in valid]) != 0:
...
class Database:
    ...
    def __init__(self, for_read=False):
        ...
        for line in file(self.shadow_path):
            if line != "" and line != "\n":
                parts = line.rstrip("\n").split(":")
                if parts[0] in self.users_by_name:
...
def userList():
    ...
    return [format(db.users, x) for x in db.users]
...
def userInfo(uid):
    ...
    if uid in db.users:
        u = db.users[uid]
        groups = []
        for item in list(db.groups.keys()):
...
def groupList():
    ...
    db = Database(for_read=True)
    return [format(db.groups, x) for x in db.groups]
